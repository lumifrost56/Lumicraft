...
unsigned int get_block(glm::ivec3 world_pos) {
    ...
    glm::ivec3 local_pos = world_pos - chunk_pos * (int)chunk_size;
...
std::set<glm::ivec3, ivec3_compare> chunks_to_finish;
...
void chunkloader_worker() {
    while (chunkloader_worker_running) {
        ...
        for (int i = 0; i < chunk_size; i++) {
            for (int j = 0; j < chunk_size; j++) {
                chunk.blocks[i][0][j] = OAKLOG;
            }
        }
        for (int i = 0; i < chunk_size; i++) {
            for (int j = 0; j < chunk_size; j++) {
                chunk.blocks[i][j][0] = STONE;
            }
        }
        for (int i = 0; i < chunk_size; i++) {
            for (int j = 0; j < chunk_size; j++) {
                chunk.blocks[0][i][j] = ICE;
            }
        }
...
void update_chunks(glm::vec3 camera_pos) {
    ...
    for (auto &chunk_pos : required) {
        if (!chunks.count(chunk_pos) && !chunks_to_finish.count(chunk_pos)) {
            {
                std::lock_guard lock(queue_mutex);

                chunks_to_generate.push(chunk_pos);

                chunks_to_finish.insert(chunk_pos);
            }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        chunks_to_finish.erase(chunk_pos);
