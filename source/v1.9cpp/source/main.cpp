...
const unsigned int scrwidth = 1500;
...
int main() {
    ...
    float px = 11.0f/scrwidth;
    float py = 11.0f/scrheight;

    float crosshairverts[] = {-px, 0.0f, px, 0.0f, 0.0f, -py, 0.0f, py};
    ...
    Shader crosshairshader("../source/shaders/crosshair.vs", "../source/shaders/crosshair.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera.fov), (float)scrwidth / (float)scrheight, 0.1f, 200.0f);
...
void processinput(GLFWwindow *window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) {
        camera.blockselectedid = 1;
    }
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) {
        camera.blockselectedid = 4;
    }
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) {
        camera.blockselectedid = 5;
    }
    if (glfwGetKey(window, GLFW_KEY_4) == GLFW_PRESS) {
        camera.blockselectedid = 6;
    }
}