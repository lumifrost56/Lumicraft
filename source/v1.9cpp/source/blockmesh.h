...
glm::vec2 rotateUV90(glm::vec2 uv) {
    return glm::vec2(uv.y, 1.0f - uv.x);
}

glm::vec2 rotateUV180(glm::vec2 uv) {
    return glm::vec2(1.0f - uv.x, 1.0f - uv.y);
}

glm::vec2 rotateUV270(glm::vec2 uv) {
    return glm::vec2(1.0f - uv.y, uv.x);
}

glm::vec2 mirrorHorizontal(glm::vec2 uv) {
    return glm::vec2(1.0f - uv.x, uv.y);
}

glm::vec2 mirrorVertical(glm::vec2 uv) {
    return glm::vec2(uv.x, 1.0f - uv.y);
}

glm::vec2 mirrorBoth(glm::vec2 uv) {
    return glm::vec2(1.0f - uv.x, 1.0f - uv.y);
}

void generateblock(std::vector<Vertex> &vertices, std::vector<unsigned int> &indices, std::vector<unsigned int> texids, int flip = 0) {
    ...
    if (flip == 1) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                glm::vec2 texcoor = vertices[i*4+j].texcoor;
                texcoor = rotateUV180(texcoor);
                vertices[i*4+j].texcoor = texcoor;
            }
        }
    }
...
void loadblockmeshes() {
    ...
    generateblock(vertices, indices, {0, 0, 0, 0, 1, 1}, 0);
    Blockmesh oaklog1(vertices, indices);
    blockmeshes.push_back(oaklog1);

    generateblock(vertices, indices, {1, 1, 0, 0, 0, 0}, 2);
    Blockmesh oaklog3(vertices, indices);
    blockmeshes.push_back(oaklog3);

    generateblock(vertices, indices, {0, 0, 1, 1, 0, 0}, 4);
    Blockmesh oaklog5(vertices, indices);
    blockmeshes.push_back(oaklog5);

    generateblock(vertices, indices, {2, 2, 2, 2, 2, 2});
    Blockmesh stone(vertices, indices);
    blockmeshes.push_back(stone);

    generateblock(vertices, indices, {3, 3, 3, 3, 3, 3});
    Blockmesh ice(vertices, indices);
    blockmeshes.push_back(ice);

    generateblock(vertices, indices, {4, 4, 4, 4, 4, 4});
    Blockmesh tuff(vertices, indices);
    blockmeshes.push_back(tuff);
}
#endif