...
void loadtextures(std::vector<Texture> textures) {
    ...
    const int layers = textures.size();
    unsigned char *data = stbi_load(textures[0].path.c_str(), &width, &height, &nchannels, 4);
    ...
    for (int i = 1; i < layers; i++) {
        unsigned char *data = stbi_load(textures[i].path.c_str(), &width, &height, &nchannels, 4);
